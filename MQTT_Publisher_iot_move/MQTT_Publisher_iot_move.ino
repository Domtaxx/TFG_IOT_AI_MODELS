...
void reconnect() {
  while (!client.connected()) {
    ...
    if (client.connect("ESP32Client_iot_move", mqtt_username, mqtt_password)) {
...
void loop() {
  ...
  float num = random(0, 10001) / 10000.0;  // Generate random number between 0-1
  char message[10];
  dtostrf(num, 1, 6, message);
  
  Serial.print("Publishing: ");
  Serial.println(message);

  client.publish("iot/move", message);
  delay(7000);  // Publish every 5 seconds
}
